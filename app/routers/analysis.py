...
async def get_categories():
    ...
    history = db.load_score_history(hours=6)
...
async def get_category_posts(category_id: int, hours: int = 24, limit: int = 50):
    ...
    posts = [
        ...
        for p in db.load_recent_posts(hours)
...
async def get_topic_posts(topic_id: int, q: str = "", sort: str = "created_at",
    ...
    창 제한 없음: 지난 알림 이력에서 들어와도 당시 글들이 보인다.
    """
...
async def get_alerts():
    ...
    return {
        ...
        "history": db.load_alerts(limit=100),
    }
...
async def get_snapshots(from_: str | None = Query(None, alias="from"),
    ...
    return {"snapshots": await asyncio.to_thread(db.list_snapshots, from_, to)}


@router.get("/snapshot")
async def get_snapshot(at: str | None = None):
    """특정 시각의 대시보드 전체 상태 복원 (at 이하 최신, 없으면 가장 최근)."""
    snap = await asyncio.to_thread(db.load_snapshot, at)
    if snap is None:
...
async def get_history_range(from_: str = Query(..., alias="from"), to: str = Query(...)):
    ...
    return await asyncio.to_thread(db.range_summary, from_, to)
...
async def get_recent_posts(hours: int = 1, limit: int = 50):
    """최근 수집 글 조회 (임베딩 벡터 제외)."""
    posts = db.load_recent_posts(hours)
    posts = posts[-limit:][::-1]
    return [
...
async def get_gemma_analyses(hours: int = 24, limit: int = 200):
    """글 단위 분석(adapters4) 결과 + 큐 상태."""
    return {
        "queue": db.analysis_queue_stats(),
        "results": db.load_analyses(hours, limit),
    }
